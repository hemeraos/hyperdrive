...
constexpr const char *pathToAstarteStoreDir() { return "/var/lib/astarte/crypto"; }
constexpr const char *pathToAstartePrivateKey() { return "/var/lib/astarte/crypto/astartekey.pem"; }
constexpr const char *pathToAstartePublicKey() { return "/var/lib/astarte/crypto/astartekey.pub"; }
constexpr const char *pathToAstarteCSR() { return "/var/lib/astarte/crypto/astartekey.csr"; }
...
{
    return new ThreadedKeyOperation(QLatin1String(hardwareId), QLatin1String(pathToAstartePrivateKey()),
                                    QLatin1String(pathToAstartePublicKey()), QLatin1String(pathToAstarteCSR()));
}
...
{
    bool newAvailable = QFile::exists(QLatin1String(pathToAstarteCSR())) && QFile::exists(QLatin1String(pathToAstartePublicKey()));
    if (newAvailable && QFile::exists(QLatin1String(pathToAstartePrivateKey()))) {
        if (pkey) {
            // Free the previous key
            EVP_PKEY_free(pkey);
        }

        FILE *file = fopen(pathToAstartePrivateKey(), "r");
        pkey = PEM_read_PrivateKey(file, NULL, 0, NULL);
        fclose(file);
        newAvailable = pkey;
    }
    ...
    : Hemera::AsyncInitObject(*new CryptoPrivate(this), parent)
{
    ...
    // Does the dir exist?
    if (!QFileInfo::exists(QLatin1String(pathToAstarteStoreDir()))) {
        // Create
        qCDebug(astarteCryptoDC) << "Creating Astarte Crypto store directory";
        QDir cryptoDir;
        if (!cryptoDir.mkpath(QLatin1String(pathToAstarteStoreDir()))) {
...
{
    return QLatin1String(pathToAstarteCSR());
}

QString Crypto::pathToPrivateKey()
{
    return QLatin1String(pathToAstartePrivateKey());
}

QString Crypto::pathToPublicKey()
{
    return QLatin1String(pathToAstartePublicKey());
}
